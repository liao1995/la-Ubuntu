...
%   argument list, a single random number of the specified TYPE and
%   default parameters shown below is generated. If only TYPE, M, 
%   and N are provided, the default parameters shown below are used.
%   If M = N = 1, IMNOISE2 generates a single random number of the
%   specified TYPE and parameters A and B.
%
...
%                   probability Pa = A, and amplitude 1 with probability
%                   Pb = B. The default values are Pa = Pb = A = B = 
%                   0.05. Note that the noise has values 0 (with 
%                   probability Pa = A) and 1 (with probability Pb = 
%                   B), so scaling is necessary if values other than
%                   0 and 1 are required. The noise matrix R is assigned
%                   three values. If R(x, y) = 0, the noise at (x, y)
%                   is pepper(black). If R(x, y) = 1, the noise at (x, y)
%                   is salt(white). If R(x, y) = 0.5, there is no noise
...
% The coordinates of these points we call 0 (pepper noise). Similiarly,
% Pb*(M*N) points will have values in the range > a & <= (a+b). These
% we call 1 (salt noise).
X = rand(M, N);
R(X <= a) = 0;
u = a + b;
R(X > a & X <= u) = 1;
